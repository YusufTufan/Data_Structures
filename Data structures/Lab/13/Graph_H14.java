public class Graph<T extends Comparable> {
    ...
    public void DFS(T startingVertexID) throws Exception {
        Vertex<T> startVertex=findVertex(startingVertexID);
        GenericStack<Vertex> stk=new GenericStack<>(Vertex.class, 100);
        LinkedList<Vertex> visited=new LinkedList<>();
        stk.push(startVertex);
        while(!stk.isEmpty()){
            Vertex current=stk.pop();
            if(visited.search(current)) {
                System.out.println(visited.search(current));
                continue;
            }else {
                visited.addToEnd(current);
                Edge<T> iteratorEdge = current.edgeLink;
                while(iteratorEdge!=null) {
                    if(!visited.search(findVertex(iteratorEdge.vertexID)))
                        stk.push(findVertex(iteratorEdge.vertexID));
                    iteratorEdge=iteratorEdge.nextEdge;
                }
            }
        }
    ...
    public void BFS(T startingVertexID) throws Exception {
        Vertex<T> startVertex = findVertex(startingVertexID);
        CircularQueue<Vertex> queue=new CircularQueue<>(Vertex.class, this.vertexCount()+1);
        LinkedList<Vertex> visited=new LinkedList<>();
        ...
        while(!queue.isEmpty()){
            ...
            if(visited.search(current)) {
                ...
                Edge<T> iteratorEdge = current.edgeLink;
                while(iteratorEdge!=null) {
                    queue.enQueue(findVertex(iteratorEdge.vertexID));
                    iteratorEdge=iteratorEdge.nextEdge;
                }
            }
        }
